...
const Sidenav = ({ innerRef, startLoading }) => {
    ...
    return (
        <div ref={innerRef} className="tw--translate-x-64 md:tw-translate-x-0 tw-z-20 tw-overflow-auto tw-w-64 tw-h-full tw-bg-sky-100 tw-fixed tw-transition-all tw-duration-500 tw-ease-in-out">
...

                                {
                                    m.subMenu === null ?
                                        ...
                                        <Link onClick={startLoading} to={m.link} state={{ title: m.title }} className={`${headerBar.title === m.id ? 'tw-text-white hover:tw-text-white' : 'tw-text-gray-700 hover:tw-text-gray-700'} tw-relative tw-flex parent tw-items-center tw-py-2 tw-px-6 tw-overflow-hidden tw-text-ellipsis tw-text-sm tw-whitespace-nowrap tw-transition tw-duration-300 tw-ease-in-out`}>
                                            ...
                                            <span className={` ${headerBar.title === m.id ? 'tw-bg-red-500' : 'tw-bg-sky-100 parent-hover:myHover'} tw--z-10 tw-rounded tw-left-4 tw-right-4 tw-h-8 tw-absolute tw-transition-all tw-duration-300`}></span>
                                        </Link>
                                        :
                                        <button id={m.id} onClick={subNavHandler} className={`tw-text-gray-700 tw-relative parent expanded tw-flex tw-w-full tw-items-center tw-py-2 tw-px-6 tw-overflow-hidden tw-text-sm tw-text-ellipsis tw-whitespace-nowrap tw-rounded-lg tw-transition tw-duration-300 tw-ease-in-out`}>
                                            {<m.icon />}
                                            <span className="tw-ml-2 tw-font-semibold">{m.title}</span>
                                            <MdKeyboardArrowRight className={`${subMenuId === m.id ? 'tw-rotate-90' : 'tw-rotate-0'} tw-ml-auto tw-transition tw-duration-300`} />
                                            <span className={` tw--z-10 tw-rounded tw-left-4 tw-right-4 parent-hover:myHover tw-h-8 tw-absolute tw-transition-all tw-duration-300`}></span>
                                        </button>
                                            ...
                                            m.subMenu.map((sm) => {
                                                return (
                                                    <li key={sm.id} className="tw-relative">
                                                        ...
                                                        <Link onClick={startLoading} to={sm.link} state={{ title: m.title, subtitle: sm.title }} className={`${headerBar.subtitle === sm.id ? 'tw-text-white hover:tw-text-white' : 'tw-text-gray-700 hover:tw-text-gray-700'} tw-flex tw-relative parent tw-items-center tw-py-2 tw-px-4 tw-overflow-hidden tw-text-gray-700 tw-text-sm tw-text-ellipsis tw-whitespace-nowrap tw-transition tw-duration-300 tw-ease-in-out`}>
                                                            <span className="tw-ml-2 tw-font-medium">{sm.title}</span>
                                                            <span className={`${headerBar.subtitle === sm.id ? 'tw-bg-red-500' : 'tw-bg-sky-100 parent-hover:myHover'} tw--z-10  tw-left-4 tw-right-4 tw-rounded tw-h-8 tw-absolute tw-transition-all tw-duration-300`}></span>
